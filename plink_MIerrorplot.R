...
postscript("plink_MIerrorplot.ps", paper="letter", horizontal=T, fonts=c("serif", "Palatino"))
par(mfrow=c(1, 2))
for (famid in unique(mi.err)){
  ...
  mtext(paste("Documented versus Inferred Family", famid, "in plink"), side = 3, line = -2, outer = TRUE)}
dev.off()
rm(list=ls(all=TRUE))
