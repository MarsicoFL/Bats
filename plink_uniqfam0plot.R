## plink_uniqfam0plot.R for KING --related, by Wei-Min Chen and Zhennan Zhu
rm(list=ls(all=TRUE))
library(igraph)
data <- read.table(file="plink.kin0", header=TRUE, stringsAsFactors=FALSE)[,c("ID1", "ID2", "InfType")]
...
postscript("plink_uniqfam0plot.ps", paper="letter", horizontal=T)
plot(uniq.cluster, vertex.color=NA, vertex.size=1, vertex.label=NA, layout=lo, asp=0,
  edge.color=Inf.color[as.numeric(E(uniq.cluster)$color)], main="All Unique Family Configurations in plink")
...
par(mfrow=c(2,3))
is.built <- all.builds!=""
for(i in uniq.fam.info[, 1][is.built]){
  index <- (1:length(uniq.fam.info[, 1]))[uniq.fam.info[, 1]==i]
  g1 <- induced_subgraph(g, (1:length(V(g)))[imc.membership == i])
  if(substr(all.builds[index],1,12)!="Undetermined"&&all.counts[index]>1||all.counts[index]>10)
  plot(g1, vertex.color=NA, vertex.size=3, vertex.label=NA, layout=layout_with_fr, asp=0,
  edge.color=Inf.color[as.numeric(edge.attributes(g1)$color)], main=paste(all.counts[index], all.builds[index], "Families"))}
dev.off()
rm(list=ls(all=TRUE))
